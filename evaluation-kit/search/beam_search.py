...
@sgl.function
def beam_search(s: ProgramState, question, prompt, verifier: Callable[[str, List[str]], List[List[float]]], tokenizer: PreTrainedTokenizer, args: SearchConfig):
    s += prompt
    ...
    # completed_beams = sorted(completed_beams, key=lambda x: x["agg_scores"], reverse=True)
    ...
    s["output"] = outputs[np.argmax(agg_scores)]
    s["outputs"] = outputs
    s["scores"] = scores
